...
class Settings():
    def __init__(self, temperature, humid_dewpt, apparentTemp_heatIndx, wind, precip, visibility, transport, snow, ceilingMixing, fire, ft20, lightning, celsius, fahrenheit, kelvin, theme):
...
def sql_settings_make_table(con):  # Run on startup of program to ensure there's a table.
...
def sql_settings_gui_insert(con, settings_obj): # Feed this thing the settings object
...
def sql_settings_gui_get(con): # returns list of settings
    cursorObj = con.cursor()
    cursorObj.execute("SELECT * from settings")
    rows = cursorObj.fetchall()
    rowList = [row for row in rows]
...
# s = sql_settings_gui_get(con)

# sql_settings_gui_insert(con, s)
# s = sql_settings_gui_get(con)
...
def startup(): # This is a simulated main loop. This will actually go into the qt application via importing this file and calling startup there.
    try:
        ...
        data_pull(user.get_weather_data)
    except:
        print("error")
